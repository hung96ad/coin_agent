...
from modelDB import SQLALCHEMY_DATABASE_URI, Trade, CoinInfo, db
...
with open('model_retrain.pkl', 'rb') as fopen:
...
for _, symbol in symbols.iterrows():
    df = pd.read_sql(query.format(symbol.id, 0), SQLALCHEMY_DATABASE_URI)
